...
const frequentlyBoughtItems = async (req, res, next) => {
  const { cust_no } = req;
  try {
    const [frequent, metadata] =
      await sequelize.query(`select distinct t_item.id,count(*),t_order.cust_no, t_item.name,t_item.brand_id,t_item.UOM ,t_item.is_grocernest,t_item.category_id ,t_item.sub_category_id ,
    t_item.image ,t_item.description ,t_item.available_for_ecomm ,t_batch.batch_no ,
    t_batch.location_id ,t_batch.MRP ,t_batch.discount ,t_batch.cost_price ,t_batch.mfg_date ,t_batch.sale_price ,
    t_batch.created_at,t_lkp_color.color_name, t_lkp_brand.brand_name, t_lkp_category.group_name, t_batch.mark_selected,t_batch.id as "batch_id"
    from (((((((t_order
          ...
          inner join t_lkp_brand on t_lkp_brand.id = t_item.brand_id)
          inner join t_inventory on t_inventory.item_id = t_item.id)
          where t_inventory.location_id = 4 and t_inventory.balance_type = 1 and t_lkp_category.available_for_ecomm = 1 and t_item.available_for_ecomm = 1 and t_batch.mark_selected = 1 and t_order.cust_no = '${cust_no}'
    ...
    if (frequent.length === 0) {
      return res.status(200).send({
        success: true,
        data: [],
        message: "There are no frequently bought items to show right now",
      });
    }

    const promises = frequent.map(async (current) => {
      let itemInWishlist;
      if (currentUser) {
        itemInWishlist = await WishlistItems.findOne({
          where: { cust_no: currentUser, item_id: current.id },
    ...
    return res.status(200).send({
      success: true,
      data: responseArray,
      message: "Found all the frequently bought items",
    });
  } catch (error) {
    return res.status(400).send({
      success: false,
