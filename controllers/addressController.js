...
const getAllAddresses = async (req, res, next) => {
  ...
  try {
    const addresses = await Address.findAll({ where: { cust_no } });
    ...
    if (addresses.length === 0) {
      return res.status(200).send({
        ...
        data: [],
        message:
          "There are no addresses available for you right now, consider adding one",
      });
    }
    ...
    return res.status(200).send({
      success: true,
      data: addresses,
      message: "Found all addresses for current user",
    });
  } catch (error) {
    return res.status(400).send({
      ...
      data: error.message,
      message:
        "Something went wrong while fetching addresses, please try again in sometime",
    });
  }
};
