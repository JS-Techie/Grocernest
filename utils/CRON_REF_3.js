            ...
            invoice_items.map(async (current_item) => {
                ...
                if (inventory_item.cashback) {
                    ...
                    if (inventory_item.cashback_is_percentage == 1) {
                        cashback_amt = ((current_item.sale_price / 100) * inventory_item.cashback).toFixed(2);
                    }
                    else {
                    ...
                    let this_invoice = await Invoice.findOne({ where: { id: current_invoice.id } })
                    ...
                    if (cust.password)
                        await walletService.creditAmount(cashback_amt, cust.cust_no, "Cashback added for Store Purchase INV/PREFIX/" + current_invoice.id);

                    await Invoice.update(
                        {
                            cashback_processed: "1",
                            cashback_amount: parseFloat(this_invoice.cashback_amount ? this_invoice.cashback_amount : 0) + parseFloat(cashback_amt)
                        },
                        {
                            where: {
